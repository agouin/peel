...
/// Drive the hand-rolled HTTP client end-to-end against a localhost
/// HTTP/1.1 mock with a UringBackend, covering the §7b.3 plumbing
/// (`Client::with_backend` → uring `connect/recv/send`).
#[test]
fn http_client_uses_uring_backend_for_socket_io() {
    ...
    let server = std::thread::spawn(move || {
        // Accept one connection and serve a single canned HEAD response.
        let (mut s, _) = listener.accept().expect("accept");
        let mut reader = BufReader::new(s.try_clone().expect("clone"));
        // Read the request line + headers; we only care that some
        // request arrived. Stop on the empty line.
        loop {
        ...
        s.write_all(resp.as_bytes()).expect("write headers");
        s.write_all(body).expect("write body");
        // Drop closes; client's body reader sees EOF after Content-Length.
        let _ = s;
    });
    ...
    let client =
        Client::with_config_and_backend(cfg, Arc::clone(&backend)).expect("client constructs");
    assert_eq!(client.io_backend_name(), "uring");

    let url = Url::parse(&format!("http://127.0.0.1:{port}/test")).expect("parse url");
    let head = client.head(&url).expect("HEAD ok");
    assert_eq!(head.status.code, 200);
    assert_eq!(head.headers.get("content-length"), Some("16"));

    // Now follow up with a full GET to exercise the recv path; reusing
    // the same Client opens a fresh connection because the server
    // closed after the HEAD.
    // Server thread already exited; spin up another for the GET.
    let _ = server.join();

    let listener2 = TcpListener::bind(("127.0.0.1", 0)).expect("bind get");
    let addr2 = listener2.local_addr().expect("local_addr get");
    let port2 = addr2.port();
    let server2 = std::thread::spawn(move || {
        let (mut s, _) = listener2.accept().expect("accept");
        let mut reader = BufReader::new(s.try_clone().expect("clone"));
        loop {
            let mut line = String::new();
            let n = reader.read_line(&mut line).expect("read_line");
    ...
    let url2 = Url::parse(&format!("http://127.0.0.1:{port2}/")).expect("parse url");
    let mut resp = client.get_full(&url2).expect("GET ok");
    let mut body = Vec::new();
    resp.body.read_to_end(&mut body).expect("read body");
    assert_eq!(body, b"hello peel uring");
    server2.join().expect("server2");
}
