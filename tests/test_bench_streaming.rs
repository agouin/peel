...
// ---- Phase 8 of `docs/PLAN_xz_liblzma_port.md`: bench grid `tar.xz`
// row under both decoders -----------------------------------------------

/// Construct a [`DecoderRegistry`] that re-points the `.xz` /
/// `.tar.xz` suffixes (and the xz magic) at
/// [`peel::decode::xz_liblzma::factory`] instead of the
/// production [`peel::decode::xz_native::factory`]. Other formats
/// (zstd, gzip, lz4, tar) keep their default factories.
///
/// `register_format` replaces existing entries by name / suffix /
/// magic (per the dedupe path in
/// [`DecoderRegistry::register`] / [`register_name`] /
/// [`register_magic`]), so this is an in-place swap.
fn registry_with_xz_liblzma() -> DecoderRegistry {
    use peel::decode::MagicSignature;
    ...
    r.register_format(
        ...
        peel::decode::xz_liblzma::factory,
    );
    r
}

/// 3-way bench grid for the `tar.xz` row: at each of the four
/// rate cells (10 Mbps / 100 Mbps / 1 Gbps / 10 Gbps), time
/// `peel-native` (production xz_native), `peel-port`
/// (xz_liblzma), and the `curl|xz|tar` baseline. Print the two
/// peel ratios side-by-side per row so the integration-vs-shelf
/// decision is one glance.
///
/// Phase 9 of [`docs/PLAN_xz_liblzma_port.md`] consumes this
/// row's headline `1 Gbps · 128 MiB` ratio as the integration
/// signal: if peel-port lands ≤ peel-native (or close), the
/// port is worth integrating; if the framing-tax shifts make
/// it materially slower, the port stays as a benchmarking
/// reference.
#[test]
#[ignore = "benchmark; opt-in via --ignored"]
fn bench_throttled_xz_liblzma_compare_grid() {
    if !tool_present("curl") {
        skip("net", "curl");
        return;
    }
    if !tool_present("xz") || !tool_present("tar") {
        skip("tar.xz", "xz/tar");
        return;
    }
    ...
    println!(
        "[xz-port] {:>10}  {:>9}  {:>10}  {:>10}  {:>11}  {:>14}  {:>14}",
        "rate", "payload", "peel-native", "peel-port", "curl|xz|tar", "native/base", "port/base",
    );

    for rate in rates {
        let (archive, entries) = build_tar_payload_sized(rate.payload_bytes);
        let body = encode_xz(&archive);
        let payload_mib = (rate.payload_bytes as f64) / (1024.0 * 1024.0);
        let suffix = "bundle.tar.xz";

        let server = MockServer::start(ok_handler(body.clone()));
        let url = format!("{}/{suffix}", server.base_url());

        // ---- peel + xz_native (default registry, production path) ----
        let work_native = unique_dir("xz_native_grid");
        let _g_native = CleanupDir(work_native.clone());
        let mut config = coord_config();
        config.max_bandwidth_bps = Some(rate.bytes_per_sec);
        let progress_state = std::sync::Arc::new(peel::progress::ProgressState::new());
        let args_native = RunArgs {
            url: url.clone(),
            additional_urls: Vec::new(),
            output: OutputTarget::Dir(work_native.clone()),
            config,
            client: build_client(),
            registry: DecoderRegistry::with_defaults(),
            progress: None,
        ...
        let stats_native = run(args_native).expect("peel-native run");
        let native_elapsed = started_native.elapsed();
        assert_eq!(stats_native.extraction.bytes_out, archive.len() as u64);
        assert_dir_matches(&work_native, &entries);

        // ---- peel + xz_liblzma (port registry) ----
        let work_port = unique_dir("xz_port_grid");
        let _g_port = CleanupDir(work_port.clone());
        let mut config = coord_config();
        config.max_bandwidth_bps = Some(rate.bytes_per_sec);
        let progress_state2 = std::sync::Arc::new(peel::progress::ProgressState::new());
        let args_port = RunArgs {
            url: url.clone(),
            additional_urls: Vec::new(),
            output: OutputTarget::Dir(work_port.clone()),
            config,
            client: build_client(),
            registry: registry_with_xz_liblzma(),
            progress: None,
            progress_state: Some(std::sync::Arc::clone(&progress_state2)),
            kill_switch: None,
            io_backend: None,
        };
        let started_port = Instant::now();
        let stats_port = run(args_port).expect("peel-port run");
        let port_elapsed = started_port.elapsed();
        assert_eq!(stats_port.extraction.bytes_out, archive.len() as u64);
        assert_dir_matches(&work_port, &entries);

        // ---- curl|xz|tar baseline ----
        let work_b = unique_dir("xz_curl_grid");
        let _g_b = CleanupDir(work_b.clone());
        let pipeline = format!(
            r#"curl -sS --limit-rate {limit} "$URL" | xz -d -q | tar -xf - -C {dir}"#,
            limit = rate.bytes_per_sec,
            dir = shell_quote(&work_b),
        );
        let base_elapsed = time_pipeline(&url, &pipeline);
        assert_dir_matches(&work_b, &entries);

        let n_s = native_elapsed.as_secs_f64();
        let p_s = port_elapsed.as_secs_f64();
        let b_s = base_elapsed.as_secs_f64();
        println!(
            "[xz-port] {rate:>10}  {payload:>6.1} MiB  {native:>8.3}s  {port:>8.3}s  {base:>9.3}s  {native_ratio:>11.2}x  {port_ratio:>11.2}x",
            rate = rate.label,
            payload = payload_mib,
            native = n_s,
            port = p_s,
            base = b_s,
            native_ratio = n_s / b_s.max(1e-9),
            port_ratio = p_s / b_s.max(1e-9),
        );
    }
}
